...
class ALUTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "perform ADD operation" in {
    test(new ALU) { dut =>
      dut.io.op_a.poke(15.U)
      dut.io.op_b.poke(10.U)
      dut.io.alu_op.poke(0.U)  // ADD
      dut.io.result.expect(25.U)

      // Edge case: overflow wraps around
      dut.io.op_a.poke("hFFFFFFFF".U)
      dut.io.op_b.poke(1.U)
  ...
  it should "perform AND operation" in {
    test(new ALU) { dut =>
      dut.io.op_a.poke("b1010".U)
      dut.io.op_b.poke("b1100".U)
      dut.io.alu_op.poke(2.U)  // AND
      dut.io.result.expect("b1000".U)

      dut.io.op_a.poke("hFF00FF00".U)
      dut.io.op_b.poke("h00FFFF00".U)
      dut.io.result.expect("h00FF0000".U)
    }
  }
  ...
  it should "perform XOR operation" in {
    test(new ALU) { dut =>
      ...
      // XOR with self = 0
      dut.io.op_a.poke(12345.U)
      dut.io.op_b.poke(12345.U)
      dut.io.result.expect(0.U)
    }
  }
  ...
  it should "perform SLL (shift left logical)" in {
    test(new ALU) { dut =>
      dut.io.op_a.poke("b0001".U)
      dut.io.op_b.poke(2.U)  // Shift amount
      dut.io.alu_op.poke(5.U)  // SLL
      dut.io.result.expect("b0100".U)

      dut.io.op_a.poke(1.U)
      dut.io.op_b.poke(31.U)  // Maximum shift
      dut.io.result.expect("h80000000".U)

      // Only lower 5 bits of shift amount used (RISC-V spec)
      dut.io.op_a.poke(1.U)
      dut.io.op_b.poke(33.U)  // Should be treated as 1
  ...
  it should "perform SRA (shift right arithmetic)" in {
    test(new ALU) { dut =>
      ...
      dut.io.op_a.poke("b0100".U)
      dut.io.op_b.poke(1.U)
      dut.io.alu_op.poke(7.U)  // SRA
      dut.io.result.expect("b0010".U)

      // Negative number - sign extends
      dut.io.op_a.poke("h80000000".U)  // -2147483648 in signed
      dut.io.op_b.poke(1.U)
      dut.io.result.expect("hC0000000".U)  // Sign bit preserved

      dut.io.op_a.poke("hFFFFFFFC".U)  // -4 in signed
      dut.io.op_b.poke(1.U)
  ...
  it should "perform SLT (set less than - signed)" in {
    test(new ALU) { dut =>
      ...
      // 10 < 5 = false
      dut.io.op_a.poke(10.U)
      dut.io.op_b.poke(5.U)
      dut.io.result.expect(0.U)

      // Signed comparison: -1 < 0
      dut.io.op_a.poke("hFFFFFFFF".U)  // -1 in signed
      dut.io.op_b.poke(0.U)
      dut.io.result.expect(1.U)

      // Equal values = false
      dut.io.op_a.poke(42.U)
      dut.io.op_b.poke(42.U)
      dut.io.result.expect(0.U)
    }
  }
  ...
  it should "perform SLTU (set less than - unsigned)" in {
    test(new ALU) { dut =>
      ...
      // Opposite test
      dut.io.op_a.poke(0.U)
      dut.io.op_b.poke("hFFFFFFFF".U)
      dut.io.result.expect(1.U)
    }
  }
